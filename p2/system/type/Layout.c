...
Object create_type_with_vars(Type_Class type, uns_int size)
{
...
void pre_init_Type_Layout()
{
    ...
    empty_array_layout      = create_type_with_vars(Type_ArrayLayout, 0);
    empty_object_layout     = create_type_with_vars(Type_ObjectLayout, 0);
...
Object create_object_type(uns_int size)
{
    if (size == 0) { return empty_object_layout; }
    return create_type_with_vars(Type_ObjectLayout, size);
}

Object create_array_type(uns_int size)
{
    if (size == 0) { return empty_array_layout; }
    return create_type_with_vars(Type_ArrayLayout, size);
}

Object create_type(uns_int size, Type_Tag tag)
{
    if (tag == OBJECT)  { return create_object_type(size); }
    if (tag == ARRAY)   { return create_array_type(size); }
