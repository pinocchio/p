...
; Global definitions
(define globals (new-environment native))

; Load the input files
(let ((args (current-command-line-arguments)))
  (map (lambda (file)
         (top-level-syntax->ast
            (read-syntax file (open-input-file file))))
       (vector->list args)))

(define (top-level-syntax->ast syntax)
    (let ((code (syntax-object->datum syntax)))
