...
static void init_SMB_inspectChildrenOn_() {
    ...
    Array PArray4680 = new_Array_with(1, (Optr)VAR_inspector_0_0);
    Symbol SMB_class = new_Symbol(L"class");
    // class. 
    Send PSend4682 = new_Send((Optr)self, SMB_class, 0);
    Symbol SMB_superclass = new_Symbol(L"superclass");
    // superclass. 
    Send PSend4683 = new_Send((Optr)PSend4682, SMB_superclass, 0);
    Symbol SMB_layout = new_Symbol(L"layout");
    // layout. 
    Send PSend4684 = new_Send((Optr)PSend4683, SMB_layout, 0);
    Symbol SMB_inspectChildrenFrom_on_ = new_Symbol(L"inspectChildrenFrom:on:");
    // inspectChildrenFrom:on:. 
    Send PSend4685 = new_Send((Optr)PSend4684, SMB_inspectChildrenFrom_on_, 2, (Optr)self, (Optr)VAR_inspector_0_0);
    Array PThreadedCode4681 = instantiate_Array_with(ThreadedCode_Class, 0, 13, (Optr)&t_push_self, (Optr)&t_send0, (Optr)PSend4682, (Optr)&t_send0, (Optr)PSend4683, (Optr)&t_send0, (Optr)PSend4684, (Optr)&t_push_self, (Optr)&t_push_variable, (Optr)VAR_inspector_0_0, (Optr)&t_send2, (Optr)PSend4685, (Optr)&t_method_return);
    Method PMethod4679 = new_Method_with(PArray4680, empty_Array, empty_Array, PThreadedCode4681, 1, PSend4685);
    
    MethodClosure MC_SMB_inspectChildrenOn_ = new_MethodClosure((Method)PMethod4679, NativeMethod_Class);
...
static void init_SMB_accept_on_message_boundBy_() {
    ...
    Array PArray4687 = new_Array_with(4, (Optr)VAR_visitor_0_0, (Optr)VAR_receiver_0_1, (Optr)VAR_message_0_2, (Optr)VAR_closure_0_3);
    Symbol SMB_invokeNativeMethod_on_message_ = new_Symbol(L"invokeNativeMethod:on:message:");
    // invokeNativeMethod:on:message:. 
    Send PSend4689 = new_Send((Optr)VAR_visitor_0_0, SMB_invokeNativeMethod_on_message_, 3, (Optr)VAR_closure_0_3, (Optr)VAR_receiver_0_1, (Optr)VAR_message_0_2);
    Array PThreadedCode4688 = instantiate_Array_with(ThreadedCode_Class, 0, 11, (Optr)&t_push_variable, (Optr)VAR_visitor_0_0, (Optr)&t_push_variable, (Optr)VAR_closure_0_3, (Optr)&t_push_variable, (Optr)VAR_receiver_0_1, (Optr)&t_push_variable, (Optr)VAR_message_0_2, (Optr)&t_send3, (Optr)PSend4689, (Optr)&t_method_return);
    Method PMethod4686 = new_Method_with(PArray4687, empty_Array, empty_Array, PThreadedCode4688, 1, PSend4689);
    
    MethodClosure MC_SMB_accept_on_message_boundBy_ = new_MethodClosure((Method)PMethod4686, NativeMethod_Class);
