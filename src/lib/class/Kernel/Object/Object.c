...
static void init_SMB_removeNatives() {
    ...
    // methods. 
    Send PSend246 = new_Send((Optr)self, SMB_methods, 0);
    Variable VAR_method_1_0 = new_Variable_named(L"method", 1);
    Array PArray248 = new_Array_with(1, (Optr)VAR_method_1_0);
    Symbol SMB_code = new_Symbol(L"code");
    // code. 
    Send PSend250 = new_Send((Optr)VAR_method_1_0, SMB_code, 0);
    Symbol SMB_clearNative = new_Symbol(L"clearNative");
    // clearNative. 
    Send PSend251 = new_Send((Optr)PSend250, SMB_clearNative, 0);
    Array PThreadedCode249 = instantiate_Array_with(ThreadedCode_Class, 0, 7, (Optr)&t_push_variable, (Optr)VAR_method_1_0, (Optr)&t_send0, (Optr)PSend250, (Optr)&t_send0, (Optr)PSend251, (Optr)&t_method_return);
    Block PBlock247 = new_Block_with(PArray248, empty_Array, PThreadedCode249, 1, PSend251);
    Symbol SMB_do_ = new_Symbol(L"do:");
    // do:. 
    Send PSend252 = new_Send((Optr)PSend246, SMB_do_, 1, (Optr)PBlock247);
    Array PThreadedCode245 = instantiate_Array_with(ThreadedCode_Class, 0, 9, (Optr)&t_push_self, (Optr)&t_send0, (Optr)PSend246, (Optr)&t_push_closure, (Optr)PBlock247, (Optr)&t_send1, (Optr)PSend252, (Optr)&t_zap, (Optr)&t_method_return_self);
    Method PMethod244 = new_Method_with(empty_Array, empty_Array, empty_Array, PThreadedCode245, 2, PSend252, self);
    
    MethodClosure MC_SMB_removeNatives = new_MethodClosure((Method)PMethod244, Object_Class);
...
static void init_SMB_inspect() {
    ...
    // inspect:. 
    Send PSend255 = new_Send((Optr)PStreamInspector_classReference, SMB_inspect_, 1, (Optr)self);
    Array PThreadedCode254 = instantiate_Array_with(ThreadedCode_Class, 0, 6, (Optr)&t_push_class_reference, (Optr)PStreamInspector_classReference, (Optr)&t_push_self, (Optr)&t_send1, (Optr)PSend255, (Optr)&t_method_return);
    Method PMethod253 = new_Method_with(empty_Array, empty_Array, empty_Array, PThreadedCode254, 1, PSend255);
    
    MethodClosure MC_SMB_inspect = new_MethodClosure((Method)PMethod253, Object_Class);
...
static void init_SMB_swapInvokeNative() {
    ...
    Annotation PAnnotation258 = new_Annotation((Optr)SMB_pinocchioReflective_, 1, (Optr)SMB_swapInvokeNativeOn_message_);
    Array PArray257 = new_Array_with(1, (Optr)PAnnotation258);
    Symbol SMB_current = new_Symbol(L"current");
    // current. 
    Send PSend260 = new_Send((Optr)PThread_classReference, SMB_current, 0);
    Symbol SMB_nextInterpreter_ = new_Symbol(L"nextInterpreter:");
    // nextInterpreter:. 
    Send PSend261 = new_Send((Optr)PSend260, SMB_nextInterpreter_, 1, (Optr)self);
    Array PThreadedCode259 = instantiate_Array_with(ThreadedCode_Class, 0, 9, (Optr)&t_push_class_reference, (Optr)PThread_classReference, (Optr)&t_send0, (Optr)PSend260, (Optr)&t_push_self, (Optr)&t_send1, (Optr)PSend261, (Optr)&t_zap, (Optr)&t_method_return_self);
    ReflectionMethod PReflectionMethod256 = new_ReflectionMethod_with(empty_Array, empty_Array, PArray257, PThreadedCode259, 2, PSend261, self);
    
    MethodClosure MC_SMB_swapInvokeNative = new_MethodClosure((Method)PReflectionMethod256, Object_Class);
