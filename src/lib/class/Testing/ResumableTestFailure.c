...
static void init_SMB_sunitExitWith_() {
    ...
    Array PArray12015 = new_Array_with(1, (Optr)VAR_aValue_0_0);
    Symbol SMB_resume_ = new_Symbol(L"resume:");
    // resume:. 
    Send PSend12017 = new_Send((Optr)self, SMB_resume_, 1, (Optr)VAR_aValue_0_0);
    Array PThreadedCode12016 = instantiate_Array_with(ThreadedCode_Class, 0, 7, (Optr)&t_push_self, (Optr)&t_push_variable, (Optr)VAR_aValue_0_0, (Optr)&t_send1, (Optr)PSend12017, (Optr)&t_zap, (Optr)&t_method_return_self);
    Method PMethod12014 = new_Method_with(PArray12015, empty_Array, empty_Array, PThreadedCode12016, 2, PSend12017, self);
    
    MethodClosure MC_SMB_sunitExitWith_ = new_MethodClosure((Method)PMethod12014, Testing_ResumableTestFailure_Class);
...
static void init_SMB_isResumable() {
    ...
    Array PThreadedCode12019 = instantiate_Array_with(ThreadedCode_Class, 0, 1, (Optr)&t_method_return_true);
    Method PMethod12018 = new_Method_with(empty_Array, empty_Array, empty_Array, PThreadedCode12019, 1, true_Const);
    
    MethodClosure MC_SMB_isResumable = new_MethodClosure((Method)PMethod12018, Testing_ResumableTestFailure_Class);
