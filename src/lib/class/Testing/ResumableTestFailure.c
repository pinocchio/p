...
static void init_SMB_sunitExitWith_() {
    ...
    Array PArray12077 = new_Array_with(1, (Optr)VAR_aValue_0_0);
    Symbol SMB_resume_ = new_Symbol(L"resume:");
    // resume:. 
    Send PSend12079 = new_Send((Optr)self, SMB_resume_, 1, (Optr)VAR_aValue_0_0);
    Array PThreadedCode12078 = instantiate_Array_with(ThreadedCode_Class, 0, 7, (Optr)&t_push_self, (Optr)&t_push_variable, (Optr)VAR_aValue_0_0, (Optr)&t_send1, (Optr)PSend12079, (Optr)&t_zap, (Optr)&t_method_return_self);
    Method PMethod12076 = new_Method_with(PArray12077, empty_Array, empty_Array, PThreadedCode12078, 2, PSend12079, self);
    
    MethodClosure MC_SMB_sunitExitWith_ = new_MethodClosure((Method)PMethod12076, Testing_ResumableTestFailure_Class);
...
static void init_SMB_isResumable() {
    ...
    Array PThreadedCode12081 = instantiate_Array_with(ThreadedCode_Class, 0, 1, (Optr)&t_method_return_true);
    Method PMethod12080 = new_Method_with(empty_Array, empty_Array, empty_Array, PThreadedCode12081, 1, true_Const);
    
    MethodClosure MC_SMB_isResumable = new_MethodClosure((Method)PMethod12080, Testing_ResumableTestFailure_Class);
