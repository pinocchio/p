...
static void init_SMB_setUp() {
    ...
    Super PSuper7401 = new_Super(SMB_setUp, 0);
    Symbol SMB_new_ = new_Symbol(L"new:");
    Symbol SMB_dictSize = new_Symbol(L"dictSize");
    // dictSize. 
    Send PSend7404 = new_Send((Optr)self, SMB_dictSize, 0);
    // new:. 
    Send PSend7403 = new_Send((Optr)Dictionary_classReference, SMB_new_, 1, (Optr)PSend7404);
    Assign PAssign7402 = new_Assign((Optr)slot_PBenchmark_Collection_Dictionary_AbstractDictionary_dict, (Optr)PSend7403);
    // dictSize. 
    Send PSend7405 = new_Send((Optr)self, SMB_dictSize, 0);
    Variable VAR_i_1_0 = new_Variable_named(L"i", 1);
    Array PArray7407 = new_Array_with(1, (Optr)VAR_i_1_0);
    Symbol SMB_key_ = new_Symbol(L"key:");
    // key:. 
    Send PSend7409 = new_Send((Optr)self, SMB_key_, 1, (Optr)VAR_i_1_0);
    Symbol SMB_value_ = new_Symbol(L"value:");
    // value:. 
    Send PSend7410 = new_Send((Optr)self, SMB_value_, 1, (Optr)VAR_i_1_0);
    Symbol SMB_at_put_ = new_Symbol(L"at:put:");
    // at:put:. 
    Send PSend7411 = new_Send((Optr)slot_PBenchmark_Collection_Dictionary_AbstractDictionary_dict, SMB_at_put_, 2, (Optr)PSend7409, (Optr)PSend7410);
    Array PThreadedCode7408 = instantiate_Array_with(ThreadedCode_Class, 0, 15, (Optr)&t_push_slot, (Optr)slot_PBenchmark_Collection_Dictionary_AbstractDictionary_dict, (Optr)&t_push_self, (Optr)&t_push_variable, (Optr)VAR_i_1_0, (Optr)&t_send1, (Optr)PSend7409, (Optr)&t_push_self, (Optr)&t_push_variable, (Optr)VAR_i_1_0, (Optr)&t_send1, (Optr)PSend7410, (Optr)&t_send2, (Optr)PSend7411, (Optr)&t_method_return);
    Block PBlock7406 = new_Block_with(PArray7407, empty_Array, PThreadedCode7408, 1, PSend7411);
    SmallInt int_1 = new_SmallInt(1);
    Constant int_1_Const = new_Constant((Optr)int_1);
    Symbol SMB_to_do_ = new_Symbol(L"to:do:");
    // to:do:. 
    Send PSend7412 = new_Send((Optr)int_1_Const, SMB_to_do_, 2, (Optr)PSend7405, (Optr)PBlock7406);
    Array PThreadedCode7400 = instantiate_Array_with(ThreadedCode_Class, 0, 26, (Optr)&t_push_self, (Optr)&t_super0, (Optr)PSuper7401, (Optr)&t_zap, (Optr)&t_push1, (Optr)PAssign7402, (Optr)&t_push_class_reference, (Optr)Dictionary_classReference, (Optr)&t_push_self, (Optr)&t_send0, (Optr)PSend7404, (Optr)&t_send1, (Optr)PSend7403, (Optr)&t_assign, (Optr)&t_zap, (Optr)&t_push_1, (Optr)&t_push_self, (Optr)&t_send0, (Optr)PSend7405, (Optr)&t_push_closure, (Optr)PBlock7406, (Optr)&t_send_to_do_, (Optr)&t_continue_to_do_, (Optr)PSend7412, (Optr)&t_zap, (Optr)&t_method_return_self);
    Method PMethod7399 = new_Method_with(empty_Array, empty_Array, empty_Array, PThreadedCode7400, 4, PSuper7401, PAssign7402, PSend7412, self);
    
    MethodClosure MC_SMB_setUp = new_MethodClosure((Method)PMethod7399, Collection_Benchmark_Dictionary_Class);
