...
static void init_SMB_benchFib() {
    ...
    // fib. 
    Send PSend24669 = new_Send((Optr)int_31_Const, SMB_fib, 0);
    Array PThreadedCode24668 = instantiate_Array_with(ThreadedCode_Class, 0, 6, (Optr)&t_push1, (Optr)int_31, (Optr)&t_send0, (Optr)PSend24669, (Optr)&t_zap, (Optr)&t_method_return_self);
    Method PMethod24667 = new_Method_with(empty_Array, empty_Array, empty_Array, PThreadedCode24668, 2, PSend24669, self);
    
    MethodClosure MC_SMB_benchFib = new_MethodClosure((Method)PMethod24667, PBenchmark_Fib_Class);
