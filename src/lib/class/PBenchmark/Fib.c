...
static void init_SMB_benchFib() {
    ...
    // fib. 
    Send PSend24600 = new_Send((Optr)int_31_Const, SMB_fib, 0);
    Array PThreadedCode24599 = instantiate_Array_with(ThreadedCode_Class, 0, 6, (Optr)&t_push1, (Optr)int_31, (Optr)&t_send0, (Optr)PSend24600, (Optr)&t_zap, (Optr)&t_method_return_self);
    Method PMethod24598 = new_Method_with(empty_Array, empty_Array, empty_Array, PThreadedCode24599, 2, PSend24600, self);
    
    MethodClosure MC_SMB_benchFib = new_MethodClosure((Method)PMethod24598, PBenchmark_Fib_Class);
