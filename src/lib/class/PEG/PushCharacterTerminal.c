...
static void init_SMB_parseOn_() {
    ...
    Array PArray20086 = new_Array_with(1, (Optr)VAR_input_0_0);
    Symbol SMB_scanPushCharacter_ = new_Symbol(L"scanPushCharacter:");
    // scanPushCharacter:. 
    Send PSend20088 = new_Send((Optr)VAR_input_0_0, SMB_scanPushCharacter_, 1, (Optr)slot_PEG_Terminal_terminal);
    Array PThreadedCode20087 = instantiate_Array_with(ThreadedCode_Class, 0, 7, (Optr)&t_push_variable, (Optr)VAR_input_0_0, (Optr)&t_push_slot, (Optr)slot_PEG_Terminal_terminal, (Optr)&t_send1, (Optr)PSend20088, (Optr)&t_method_return);
    Method PMethod20085 = new_Method_with(PArray20086, empty_Array, empty_Array, PThreadedCode20087, 1, PSend20088);
    
    MethodClosure MC_SMB_parseOn_ = new_MethodClosure((Method)PMethod20085, PEG_PushCharacterTerminal_Class);
