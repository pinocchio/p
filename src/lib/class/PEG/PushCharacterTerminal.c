...
static void init_SMB_parseOn_() {
    ...
    Array PArray19957 = new_Array_with(1, (Optr)VAR_input_0_0);
    Symbol SMB_scanPushCharacter_ = new_Symbol(L"scanPushCharacter:");
    // scanPushCharacter:. 
    Send PSend19959 = new_Send((Optr)VAR_input_0_0, SMB_scanPushCharacter_, 1, (Optr)slot_PEG_Terminal_terminal);
    Array PThreadedCode19958 = instantiate_Array_with(ThreadedCode_Class, 0, 7, (Optr)&t_push_variable, (Optr)VAR_input_0_0, (Optr)&t_push_slot, (Optr)slot_PEG_Terminal_terminal, (Optr)&t_send1, (Optr)PSend19959, (Optr)&t_method_return);
    Method PMethod19956 = new_Method_with(PArray19957, empty_Array, empty_Array, PThreadedCode19958, 1, PSend19959);
    
    MethodClosure MC_SMB_parseOn_ = new_MethodClosure((Method)PMethod19956, PEG_PushCharacterTerminal_Class);
