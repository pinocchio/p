...
static void init_SMB_checkCached_() {
    ...
    Array PArray8689 = new_Array_with(1, (Optr)VAR_aClass_0_0);
    Symbol SMB_pPrimitive_plugin_ = new_Symbol(L"pPrimitive:plugin:");
    Symbol SMB_Runtime_minus_InlineCache = new_Symbol(L"Runtime.InlineCache");
    Annotation PAnnotation8691 = new_Annotation((Optr)SMB_pPrimitive_plugin_, 2, (Optr)SMB_checkCached_, (Optr)SMB_Runtime_minus_InlineCache);
    Array PArray8690 = new_Array_with(1, (Optr)PAnnotation8691);
    Array PThreadedCode8692 = instantiate_Array_with(ThreadedCode_Class, 0, 1, (Optr)&t_method_return_nil);
    NativeMethod PNativeMethod8688 = new_NativeMethod_with(PArray8689, empty_Array, PArray8690, PThreadedCode8692, 1, nil_Const);
    
    MethodClosure MC_SMB_checkCached_ = new_MethodClosure((Method)PNativeMethod8688, InlineCache_Class);
...
static void init_SMB_cache_at_() {
    ...
    Array PArray8694 = new_Array_with(2, (Optr)VAR_aMethod_0_0, (Optr)VAR_aClass_0_1);
    Symbol SMB_pPrimitive_plugin_ = new_Symbol(L"pPrimitive:plugin:");
    Symbol SMB_Runtime_minus_InlineCache = new_Symbol(L"Runtime.InlineCache");
    Annotation PAnnotation8696 = new_Annotation((Optr)SMB_pPrimitive_plugin_, 2, (Optr)SMB_cache_at_, (Optr)SMB_Runtime_minus_InlineCache);
    Array PArray8695 = new_Array_with(1, (Optr)PAnnotation8696);
    Array PThreadedCode8697 = instantiate_Array_with(ThreadedCode_Class, 0, 1, (Optr)&t_method_return_self);
    NativeMethod PNativeMethod8693 = new_NativeMethod_with(PArray8694, empty_Array, PArray8695, PThreadedCode8697, 1, self);
    
    MethodClosure MC_SMB_cache_at_ = new_MethodClosure((Method)PNativeMethod8693, InlineCache_Class);
...
// 		ifFalse: [self error: 'A Metaclass should only have one instance!']
    */
    ...
    // new:. 
    Send PSend8700 = new_Send((Optr)self, SMB_new_, 1, (Optr)int_3_Const);
    Array PThreadedCode8699 = instantiate_Array_with(ThreadedCode_Class, 0, 6, (Optr)&t_push_self, (Optr)&t_push1, (Optr)int_3, (Optr)&t_send1, (Optr)PSend8700, (Optr)&t_method_return);
    Method PMethod8698 = new_Method_with(empty_Array, empty_Array, empty_Array, PThreadedCode8699, 1, PSend8700);
    
    MethodClosure MC_SMB_new = new_MethodClosure((Method)PMethod8698, HEADER(InlineCache_Class));
