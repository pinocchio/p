...
static void init_SMB_checkCached_() {
    ...
    Array PArray8357 = new_Array_with(1, (Optr)VAR_aClass_0_0);
    Symbol SMB_pPrimitive_plugin_ = new_Symbol(L"pPrimitive:plugin:");
    Symbol SMB_Runtime_minus_InlineCache = new_Symbol(L"Runtime.InlineCache");
    Annotation PAnnotation8359 = new_Annotation((Optr)SMB_pPrimitive_plugin_, 2, (Optr)SMB_checkCached_, (Optr)SMB_Runtime_minus_InlineCache);
    Array PArray8358 = new_Array_with(1, (Optr)PAnnotation8359);
    Array PThreadedCode8360 = instantiate_Array_with(ThreadedCode_Class, 0, 1, (Optr)&t_method_return_nil);
    NativeMethod PNativeMethod8356 = new_NativeMethod_with(PArray8357, empty_Array, PArray8358, PThreadedCode8360, 1, nil_Const);
    
    MethodClosure MC_SMB_checkCached_ = new_MethodClosure((Method)PNativeMethod8356, InlineCache_Class);
...
static void init_SMB_cache_at_() {
    ...
    Array PArray8362 = new_Array_with(2, (Optr)VAR_aMethod_0_0, (Optr)VAR_aClass_0_1);
    Symbol SMB_pPrimitive_plugin_ = new_Symbol(L"pPrimitive:plugin:");
    Symbol SMB_Runtime_minus_InlineCache = new_Symbol(L"Runtime.InlineCache");
    Annotation PAnnotation8364 = new_Annotation((Optr)SMB_pPrimitive_plugin_, 2, (Optr)SMB_cache_at_, (Optr)SMB_Runtime_minus_InlineCache);
    Array PArray8363 = new_Array_with(1, (Optr)PAnnotation8364);
    Array PThreadedCode8365 = instantiate_Array_with(ThreadedCode_Class, 0, 1, (Optr)&t_method_return_self);
    NativeMethod PNativeMethod8361 = new_NativeMethod_with(PArray8362, empty_Array, PArray8363, PThreadedCode8365, 1, self);
    
    MethodClosure MC_SMB_cache_at_ = new_MethodClosure((Method)PNativeMethod8361, InlineCache_Class);
...
// 		ifFalse: [self error: 'A Metaclass should only have one instance!']
    */
    ...
    // new:. 
    Send PSend8368 = new_Send((Optr)self, SMB_new_, 1, (Optr)int_3_Const);
    Array PThreadedCode8367 = instantiate_Array_with(ThreadedCode_Class, 0, 6, (Optr)&t_push_self, (Optr)&t_push1, (Optr)int_3, (Optr)&t_send1, (Optr)PSend8368, (Optr)&t_method_return);
    Method PMethod8366 = new_Method_with(empty_Array, empty_Array, empty_Array, PThreadedCode8367, 1, PSend8368);
    
    MethodClosure MC_SMB_new = new_MethodClosure((Method)PMethod8366, HEADER(InlineCache_Class));
