...
static void init_SMB_previous_() {
    ...
    Array PArray18564 = new_Array_with(1, (Optr)VAR_anObject_0_0);
    Assign PAssign18566 = new_Assign((Optr)slot_Shell_ReadLine_EndLink_previous, (Optr)VAR_anObject_0_0);
    Array PThreadedCode18565 = instantiate_Array_with(ThreadedCode_Class, 0, 7, (Optr)&t_push1, (Optr)PAssign18566, (Optr)&t_push_variable, (Optr)VAR_anObject_0_0, (Optr)&t_assign, (Optr)&t_zap, (Optr)&t_method_return_self);
    Method PMethod18563 = new_Method_with(PArray18564, empty_Array, empty_Array, PThreadedCode18565, 2, PAssign18566, self);
    
    MethodClosure MC_SMB_previous_ = new_MethodClosure((Method)PMethod18563, Shell_ReadLine_EndLink_Class);
...
static void init_SMB_previous() {
    ...
    Array PThreadedCode18568 = instantiate_Array_with(ThreadedCode_Class, 0, 3, (Optr)&t_push_slot, (Optr)slot_Shell_ReadLine_EndLink_previous, (Optr)&t_method_return);
    Method PMethod18567 = new_Method_with(empty_Array, empty_Array, empty_Array, PThreadedCode18568, 1, slot_Shell_ReadLine_EndLink_previous);
    
    MethodClosure MC_SMB_previous = new_MethodClosure((Method)PMethod18567, Shell_ReadLine_EndLink_Class);
