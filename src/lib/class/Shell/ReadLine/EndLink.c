...
static void init_SMB_previous_() {
    ...
    Array PArray18234 = new_Array_with(1, (Optr)VAR_anObject_0_0);
    Assign PAssign18236 = new_Assign((Optr)slot_Shell_ReadLine_EndLink_previous, (Optr)VAR_anObject_0_0);
    Array PThreadedCode18235 = instantiate_Array_with(ThreadedCode_Class, 0, 7, (Optr)&t_push1, (Optr)PAssign18236, (Optr)&t_push_variable, (Optr)VAR_anObject_0_0, (Optr)&t_assign, (Optr)&t_zap, (Optr)&t_method_return_self);
    Method PMethod18233 = new_Method_with(PArray18234, empty_Array, empty_Array, PThreadedCode18235, 2, PAssign18236, self);
    
    MethodClosure MC_SMB_previous_ = new_MethodClosure((Method)PMethod18233, Shell_ReadLine_EndLink_Class);
...
static void init_SMB_previous() {
    ...
    Array PThreadedCode18238 = instantiate_Array_with(ThreadedCode_Class, 0, 3, (Optr)&t_push_slot, (Optr)slot_Shell_ReadLine_EndLink_previous, (Optr)&t_method_return);
    Method PMethod18237 = new_Method_with(empty_Array, empty_Array, empty_Array, PThreadedCode18238, 1, slot_Shell_ReadLine_EndLink_previous);
    
    MethodClosure MC_SMB_previous = new_MethodClosure((Method)PMethod18237, Shell_ReadLine_EndLink_Class);
