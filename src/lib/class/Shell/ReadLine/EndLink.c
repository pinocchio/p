...
static void init_SMB_previous_() {
    ...
    Array PArray18204 = new_Array_with(1, (Optr)VAR_anObject_0_0);
    Assign PAssign18206 = new_Assign((Optr)slot_Shell_ReadLine_EndLink_previous, (Optr)VAR_anObject_0_0);
    Array PThreadedCode18205 = instantiate_Array_with(ThreadedCode_Class, 0, 7, (Optr)&t_push1, (Optr)PAssign18206, (Optr)&t_push_variable, (Optr)VAR_anObject_0_0, (Optr)&t_assign, (Optr)&t_zap, (Optr)&t_method_return_self);
    Method PMethod18203 = new_Method_with(PArray18204, empty_Array, empty_Array, PThreadedCode18205, 2, PAssign18206, self);
    
    MethodClosure MC_SMB_previous_ = new_MethodClosure((Method)PMethod18203, Shell_ReadLine_EndLink_Class);
...
static void init_SMB_previous() {
    ...
    Array PThreadedCode18208 = instantiate_Array_with(ThreadedCode_Class, 0, 3, (Optr)&t_push_slot, (Optr)slot_Shell_ReadLine_EndLink_previous, (Optr)&t_method_return);
    Method PMethod18207 = new_Method_with(empty_Array, empty_Array, empty_Array, PThreadedCode18208, 1, slot_Shell_ReadLine_EndLink_previous);
    
    MethodClosure MC_SMB_previous = new_MethodClosure((Method)PMethod18207, Shell_ReadLine_EndLink_Class);
