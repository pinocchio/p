(letrec ((SmalltalkParser (newclass SmallParser Object (parsers id) (separators)
       ((assignmentOp (self super)
        ...
        (initializeCharacter (self super)
            ((((("[]{}()_^;$#:-|." 'asChoice) '\| (SmalltalkParser 'separators)) 
                '<= (self 'decimalDigit))
                '<= (self 'letter))
        ...
        (initializeSeparator (self super)
            (let ((p (Parser 'named: "COMMENT")))
                (p '<= ((((SmalltalkParser 'separators)  '\| (self 'commentFormat)) 'strongTimes) 'omit: #t))
                p))
        ...
        (injectCascadedMessageExpression (self super) 
            (let ((res (self 'cascadedMessageExpression)))
                ...
                (res 'semantics: (lambda (result) result))
                res
                ))
       ...
       ((separators (self super)
            (if (eq? (SmalltalkParser 'objectAt: 4) null)
                (let ((s (" \t\r\n" 'asChoice)))
                    (SmalltalkParser 'objectAt:put: 4 s)
                    s)
                (SmalltalkParser 'objectAt: 4))
          ))
       )))
