(letrec ((StringScanner (newclass StringScanner Object 
                ...
                ((pos        (self super) (self 'objectAt: 1))
                 ...
                 (skipWhitespaceWith: (self super scope) ((self 'separator) 'match:in: self scope)) 
                 (atEnd      (self super)
                              (= (self 'pos) 
                                 ((self 'string) 'size))))
                ((on:            (self super str) (self 'on:separator: str (Parser 'separators)))
                 (on:separator:  (self super str separator) (let ((result (self 'new)))
                                  (result 'string: str)
       ...
       (Parser
            (newclass Parser Expression (expression semantics
                                         extendScope separator) (separators)
                ...
                ((separators (self super)
                    (if (eq? (Parser 'objectAt: 4) null)
                        (let ((s (" \t\r\n" 'asChoice)))
                            (Parser 'objectAt:put: 4 s)
                            s)
                        (Parser 'objectAt: 4)))
